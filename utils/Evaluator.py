...
def llm_accuracy_score(query, prediction, ground_truth):
    ...
    print("\n 질문", query, "\n 정답", ground_truth, "\n 답변", prediction, "\n 판정", graded_outputs[0]['results'].strip())
    return 1 if graded_outputs[0]['results'].strip() == 'CORRECT' else 0
...
class Evaluator:
    ...
    def _update_eval_dict(self, question, label, response):
        ...
        #print("업데이트 이밸 딕", response["output"], pred)
