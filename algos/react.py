...
class ReactBase:
    def __init__(self, fewshot, model_name="gpt-3.5-turbo", max_iter=8, verbose=True): #model_name="text-davinci-002"
    ...
    def run(self, prompt):
        ...
        #수정
        if model_name in OPENAI_COMPLETION_MODELS:
            agent = create_react_agent(OpenAI(temperature=0, model_name=self.model_name), self.tools, prompt)
        elif model_name in OPENAI_CHAT_MODELS:
            agent = create_react_agent(ChatOpenAI(temperature=0, model_name=self.model_name), self.tools, prompt)
        agent.agent.llm_chain.prompt.template = self.fewshot
        '''              
 
        with get_openai_callback() as cb: #원래 이 아래로 인덴트 다 한칸씩, 마지막 리턴 직전까지
            st = time.time()
            
            response = self.agent(prompt) #기존       
            #response = AgentExecutor(agent=agent, tools=self.tools, verbose=True)  #수정
            #print문 추가
            print(response)                  
...
class ReactExtraTool(ReactBase):
    def __init__(self, model_name="gpt-4", available_tools=["Google", "Calculator"], fewshot="\n",
                 verbose=True):
        self.model_name = model_name
        self.verbose = verbose
        self.fewshot = fewshot
